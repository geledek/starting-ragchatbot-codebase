...
class TestCourseSearchTool:
    ...
    def test_successful_search_with_results(self, course_search_tool, mock_chroma_search_results):
    ...
    def test_empty_results_with_filters(self, course_search_tool):
        ...
        result = course_search_tool.execute("topic", course_name="Nonexistent", lesson_number=5)
    ...
    def test_source_link_generation_with_lesson(self, course_search_tool):
        ...
        course_search_tool.store.search = Mock(return_value=mock_results)
        course_search_tool.store.get_lesson_link = Mock(return_value="https://example.com/lesson-1")
        
        result = course_search_tool.execute("test")
        
        # Check that lesson link was requested
        course_search_tool.store.get_lesson_link.assert_called_once_with("MCP Course", 1)
    ...
    def test_source_link_generation_course_only(self, course_search_tool):
        ...
        course_search_tool.store.search = Mock(return_value=mock_results)
        course_search_tool.store.get_course_link = Mock(return_value="https://example.com/course")
        ...
        assert course_search_tool.last_sources[0]["link"] == "https://example.com/course"
...
class TestCourseOutlineTool:
    ...
    def test_successful_outline_retrieval(self, course_outline_tool, mock_chroma_catalog_results):
