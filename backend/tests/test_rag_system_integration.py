...
class TestRAGSystemIntegration:
    ...
    def test_content_query_workflow(self, mock_rag_system):
        ...
        # Setup mocks
        mock_rag_system._mock_session_manager.get_conversation_history.return_value = None
        ...
        mock_rag_system.tool_manager.get_last_sources = Mock(return_value=[
            {"display": "MCP Course - Lesson 1", "link": "https://example.com/mcp/lesson-1"}
        ])
        ...
        assert response == "MCP is a Model Context Protocol that enables rich context sharing between AI models."
...
3. [Advanced MCP Patterns](https://example.com/mcp-course/lesson-3)"""
        ...
        mock_rag_system._mock_ai_generator.generate_response.return_value = outline_response
        mock_rag_system.tool_manager.get_last_sources = Mock(return_value=[])
        
        response, sources = mock_rag_system.query("What is the outline of the MCP course?")
        
        # Verify AI generator was called with formatted outline query
        call_args = mock_rag_system._mock_ai_generator.generate_response.call_args
        assert call_args[1]["query"] == "Answer this question about course materials: What is the outline of the MCP course?"
    ...
    def test_session_management(self, mock_rag_system):
        ...
        mock_rag_system._mock_session_manager.get_conversation_history.return_value = history
    ...
    def test_query_without_session(self, mock_rag_system):
        ...
        mock_rag_system._mock_ai_generator.generate_response.return_value = "Response without session"
    ...
    def test_error_handling_in_query(self, mock_rag_system):
        ...
        mock_rag_system._mock_ai_generator.generate_response.side_effect = Exception("API Error")
    ...
    def test_source_tracking_workflow(self, mock_rag_system):
        ...
        mock_sources = [
            {"display": "MCP Course - Lesson 1", "link": "https://example.com/lesson-1"},
            {"display": "RAG Systems - Lesson 2", "link": "https://example.com/rag/lesson-2"}
        ]
...
class TestRAGSystemDocumentProcessing:
    ...
    def test_add_course_document(self, mock_rag_system_with_processing, sample_courses):
        ...
        mock_rag_system_with_processing._mock_doc_processor.process_course_document.return_value = (
            mock_course, mock_chunks
        )
        
        course, chunk_count = mock_rag_system_with_processing.add_course_document("/path/to/test.pdf")
        ...
        mock_rag_system_with_processing._mock_vector_store.add_course_metadata.assert_called_once_with(mock_course)
        mock_rag_system_with_processing._mock_vector_store.add_course_content.assert_called_once_with(mock_chunks)
    ...
    def test_add_course_document_error(self, mock_rag_system_with_processing):
        ...
        mock_rag_system_with_processing._mock_doc_processor.process_course_document.side_effect = Exception("Processing error")
        
        course, chunk_count = mock_rag_system_with_processing.add_course_document("/path/to/bad.pdf")
...
class TestRAGSystemQueryTypes:
    ...
    def test_outline_tool_execution(self, rag_with_real_tools):
        ...
        rag_with_real_tools._mock_vector_store._resolve_course_name.assert_called_once_with("Test Course")
        
        # Verify catalog was queried
        rag_with_real_tools._mock_vector_store.course_catalog.get.assert_called_once_with(ids=["Test Course"])
    ...
    def test_tool_manager_integration(self, rag_with_real_tools):
        ...
        search_result = tool_manager.execute_tool("search_course_content", query="test", course_name="Test")
        assert "[Test Course - Lesson 1]" in search_result
        
        # Test outline execution  
        outline_result = tool_manager.execute_tool("get_course_outline", course_name="Test Course")
